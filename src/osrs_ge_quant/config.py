...
@lru_cache
def load_processing_recipes() -> List[Dict[str, Any]]:
    """
    Load processing_recipes.yaml (required).
    """
    path = _find_config_file("processing_recipes.yaml")
    data = _load_yaml(path)
...
def save_settings(settings: Dict[str, Any]) -> None:
    """
    Save settings to settings.yaml and clear the load_settings cache.
    """
    path = _find_config_file("settings.yaml")
...
def save_strategies(strategies: Dict[str, Any]) -> None:
    """
    Save strategies to strategies.yaml and clear the load_strategies cache.
    """
    path = _try_find_config_file("strategies.yaml") or PROJECT_ROOT / "config" / "strategies.yaml"
    with path.open("w", encoding="utf-8") as f:
        yaml.safe_dump(strategies, f, default_flow_style=False, sort_keys=False)
